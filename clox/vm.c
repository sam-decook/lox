...
static void defineNative(const char* name, NativeFn function) {
    ...
    pop();
    pop();
    // Push and pop from the stack so the GC doesn't free them if a collection
    // is triggered.
}
...
void freeVM() {
    ...
    vm.initString = NULL; //next line will free it
    freeObjects();
}
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_DEFINE_GLOBAL: {
                ...
                pop(); //wait to pop in case a garbage collection happens
                break;
            }
            case OP_SET_GLOBAL: {
                ObjString* name = READ_STRING();
                if (tableSet(&vm.globals, name, peek(0))) {
                    tableDelete(&vm.globals, name); //delete value from tableSet
                    runtimeError("Undefined variable '%s'.", name->chars);
                    return INTERPRET_RUNTIME_ERROR;
                }
                break;
